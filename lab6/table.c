 ...
 *		to by SP.
 */
int partition(SET *sp, void **elts, int low, int high){
  void *piv = elts[high];
  int i = low -1;
  
  int j =low;
  for(; j <=high-1; j++){
    if((*sp->compare)(elts[j],piv) < 0){
      i++;
      //swap elts 
      void *temp = elts[i];
      elts[i] = elts[j];
...
void quickSort(SET* sp, void **elts, int low, int high){
  if(low < high){
    int part = partition(sp, elts, low, high);
    quickSort(sp, elts, low, part-1);
    quickSort(sp, elts, part +1, high);
  }
}
